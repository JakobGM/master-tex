...
This ambiguity needs to be resolved with a \textit{decision rule}, a rule which determines which polygon should be considered locations which are overlapped by several polygons.
...
The (somewhat naive) implementation of $\texttt{PreprocessRaster}(\mathcal{P},~\mathcal{T})$ provided above invokes $\texttt{Rasterize}(P,~B)$ a total of $|\mathcal{P}| |\mathcal{T}|$ times.
...
It should also be mentioned that \texttt{PreprocessRaster()} is a highly parallelizable process as well.
...
Denote the number of unique vertices representing the polygon $P$ as $|P|$ and assume that all polygons $P \in \mathcal{P}$ are simple polygons without any interior hulls (assumption~\ref{itm:simple}).
...
In reality assumption~\ref{itm:flat} does \emph{not} hold for our dataset, and any vertex $(x, y, z)$ rather obeys the following relationship,
%
\begin{equation*}
...
In the case of $\varepsilon \sim \mathcal{N}(0, \sigma^2)$, that is a normally distributed error term with constant, unknown variance $\sigma^2$ and zero mean, we could confidently construct a unbiased ordinary least squares predictor $\hat{\vec{\beta}}$ for $\vec{\beta}$.
\begin{align*}
  X_{xy}
  &=
  \begin{bmatrix}
