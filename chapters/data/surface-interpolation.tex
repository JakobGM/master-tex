...
That is, we can formulate $S$ in form of $N$ like this:
\begin{equation*}
  S_{i, j}
  =
  \begin{cases}
...
We can now construct two relatively independent predictors, $\hat{f}_{\mathrm{seg}}(X; \vec{\theta}_{\mathrm{seg}})$ and $\hat{f}_{\mathrm{norm}}(X; \vec{\theta}_{norm})$, which when combined form the main predictor $\hat{f}$ like so:
\begin{equation*}
...
Such \enquote{separation of concerns} has been shown to be beneficial for several model architectures.

We can now rasterize any polygon $P \in \mathcal{P}$ into two separate raster formats, $S$ and $N$, as illustrated in \cref{fig:3d-polygon-decomposition}.
When generalizing from a single three-dimensional polygon $P$ to a set of polygons $\mathcal{P}$, we can iteratively fill in the values for the same raster arrays $S$ and $N$.
\begin{framed}
  \noindent
...
This might cause some difficulties when mapping back from the raster domain, represented by $S$ and $N$, back to the original original polygon domain, represented by $\mathcal{P}$.
The exact conditions under which the surface raster format becomes non-representative will be discussed in detail in \cref{chap:post-processing}, but for now suffice it to say that a pseudo-inverse mapping, $m^{\dagger}$, will be implemented along the following lines:
It should be noted that the rasterization of $\mathcal{P}$ into a semantic segmentation raster and a surface normal raster is not a perfectly representative format.
Specifically, issues may arise when there exists two polygons which share a common edge in the two-dimensional projected space, but do \emph{not} share the.
\begin{figure}
  \centering
