...
\begin{figure}
  \centering
  \subfloat[Example of gable roof causing overlapping polygons. This public domain image has been sourced from Wikimedia~\cite{wiki:gable-roof}.]{\includegraphics[width=0.49\linewidth]{gable-roof.jpg}\label{fig:real-overlap}}
  \hspace{3em}
...
The assumption often being broken whenever small \enquote{sub-surfaces} are situated upon a main surface.
One such example are \textit{dormers}\footnote{\textit{Dormer} --- Roof surface protruding out from the main roof surface, often in order to protect a loft window.}, an example of which can be seen in \cref{fig:real-overlap}.
\cref{fig:dormer-3D} illustrates how our surface polygon dataset $\mathcal{P}$ would represent a type of dormer, namely one main roof surface (shown in red), and a dormer represented by two independent flat surfaces (shown in blue and green).
...
As mentioned earlier, this causes problems with the definition of the surface normal vector raster, $S$, and surface elevation raster, $Z$, as the choice of polygon becomes ambiguous.
This ambiguity needs to be resolved with a \textit{decision rule}.
...
The \textit{applicable polygon} is the single polygon which actually ends up being hit by sunlight at the given geographic point, overshadowing all other polygons in the polygon shadow subset.
This filtering problem can be thought of as a ray tracing problem, with rays emanating radially towards the earth's center.
The bird's view decision rule has been illustrated in \cref{fig:ray-tracing}, and \cref{fig:dormer-2D} is an example of a two-dimensional instance segmentation map generated with this rule from the three-dimensional roof structure in \cref{fig:dormer-3D}.
...
As a consequence these rasters represent just a \emph{single} polygon $P \in \mathcal{P}$.
We now want to generalize from rasters $Z_p$, $N_p$, $S_p$ representing a single polygon $P \in \mathcal{P}$, to rasters $Z$, $N$, $S$ representing an arbitrarily sized \emph{set} of polygons $\mathcal{P}$.
