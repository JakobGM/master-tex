...
As discussed in \cref{sec:surface-normal-raster-format}, any planar three-dimensional polygon $P \in \polygons$ can be decomposed into, and reconstructed from, two sub-components,
...
Start by noticing that $\beta_x$ and $\beta_y$ can be expressed in terms of the normal vector of the plane, $\vec{n} = {[n_x, n_y, n_z]}^T$, because of how it was constructed in \cref{eq:normalplanar} on \cpageref{eq:normalplanar},
\begin{equation*}
  \begin{bmatrix}
    n_x\\
    n_y\\
