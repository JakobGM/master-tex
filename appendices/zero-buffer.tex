\Cref{sec:data-types} discusses irregular vector data and how such vector features can be corrected by applying a zero buffer.
The \texttt{ST\_buffer()} PostGIS function can be applied to arbitrary geometric data with the \texttt{ogr2ogr} utility like so:
%
\begin{listing}[H]
  \caption{Zero-buffering vector dataset using \texttt{ogr2ogr}.}
  \begin{shellcode}
